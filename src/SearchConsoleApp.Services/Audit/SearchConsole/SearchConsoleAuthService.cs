...
public interface ISearchConsoleAuthService
{
...
public partial class SearchConsoleAuthService : ISearchConsoleAuthService, IScopedService
{
    private const string Scope =
        "https://www.googleapis.com/auth/webmasters.readonly https://www.googleapis.com/auth/analytics.readonly";

    private readonly IRepository<GoogleOAuthToken> _tokenRepo;
    private readonly ITokenProtector _protector;
    private readonly IConfiguration _config;
    private readonly IMemoryCache _cache;
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly ILogger<SearchConsoleAuthService> _logger;

    public SearchConsoleAuthService(
        IRepository<GoogleOAuthToken> tokenRepo,
        ITokenProtector protector,
        IConfiguration config,
        IMemoryCache cache,
        IHttpClientFactory httpClientFactory,
        ILogger<SearchConsoleAuthService> logger)
    {
    ...
    public Task<string> BuildAuthorizeUrlAsync(long customerId, string returnUrl)
    {
        ...
        _cache.Set($"sc-oauth:{state}", new StateEntry(customerId, returnUrl), TimeSpan.FromMinutes(10));
    ...
    {
        if (!_cache.TryGetValue<StateEntry>($"sc-oauth:{state}", out var entry) || entry == null)
        ...
        if (!tokenRes.IsSuccessStatusCode)
        {
            ...
            _logger.LogWarning("SC token exchange failed: {Status} {Body}", tokenRes.StatusCode, body);
        ...
        if (existing == null)
        {
            await _tokenRepo.InsertAsync(new GoogleOAuthToken
            {
    ...
    private async Task<string?> RefreshAccessTokenAsync(GoogleOAuthToken token, CancellationToken cancellationToken)
    {
        var section = GetConfig();
        ...
        if (!tokenRes.IsSuccessStatusCode)
        {
            _logger.LogWarning("SC token refresh failed for customer {CustomerId}", token.CustomerId);
            return null;
        }
    ...
    {
        var section = _config.GetSection("GoogleSearchConsole");
        var clientId = section["ClientId"] ?? _config["OAuth:google:ClientId"];
        if (string.IsNullOrWhiteSpace(clientId))
            throw new OAuthConfigurationException(OAuthSetupGuides.ForGoogleSearchConsole(_config));
